...
Purpose
-------
Apply configurable long-term reward and penalty methodology to governed NBA
standings, playoff, and franchise-lineage publications.

Usage
-----
Run ``python Scripts/generate_reports.py`` or import ``generate_reports``. Shared
input creation must occur separately in the sibling Data Collection project.
...
Important behavior:
    - Franchise lineage is supplied by Data Collection's published contract.
...
COLUMN_DEFINITIONS = {
    "Team": "Current normalized franchise name after applying the published franchise_lineage.csv contract.",
...
def _sha256(path: Path) -> str:
    """Return the SHA-256 digest for one source or implementation file."""

    return hashlib.sha256(path.read_bytes()).hexdigest()


def _safe_run_label(value: str | None) -> str:
    """Convert an optional human label into a Windows-safe folder suffix."""
...
def create_run_directory(
    ...
    """Create and return a unique timestamped directory for one immutable run."""

    local_time = run_time or datetime.now().astimezone()
    if local_time.tzinfo is None:
        local_time = local_time.astimezone()
    base_name = local_time.strftime("run_%Y_%m_%d_%H%M%S")
    label = _safe_run_label(run_name)
    if label:
    ...
    while candidate.exists():
        suffix += 1
        candidate = reports_root / f"{base_name}_{suffix:02d}"
    candidate.mkdir()
    return candidate, local_time


def write_latest_pointer(reports_root: Path, run_directory: Path, completed_at: datetime) -> Path:
    """Atomically point consumers to the newest successfully completed run."""

    pointer_path = reports_root / "latest_run.json"
    payload = {
        "schema_version": "latest_report_run_v1",
        "status": "complete",
        "run_id": run_directory.name,
        "run_directory": run_directory.relative_to(reports_root).as_posix(),
        "manifest": (run_directory / "Report_Manifest.json").relative_to(reports_root).as_posix(),
...
def write_ai_readable_outputs(
    ...
    manifest = {
        "manifest_version": "long_term_performance_rewards_run_v1",
        "project": "NBA Long-Term Performance Rewards",
        "run_id": reports_dir.name,
        "run_directory": reports_dir.name,
        "status": "complete",
        "generated_at_local": run_time.isoformat(),
        "generated_at_utc": run_time.astimezone(timezone.utc).isoformat(),
        "timezone_name": run_time.tzname(),
        "utc_offset": run_time.strftime("%z"),
        "purpose": (
            "Identify NBA teams that may qualify for long-term rewards or penalties based on "
            "sustained performance patterns."
        ),
        "source_files": {
            "standings": config["paths"]["standings"],
            "playoffs": config["paths"]["playoffs"],
            "team_mapping": config["paths"]["team_mapping"],
            "analysis_config": "Config/analysis_settings.json",
        },
        "source_sha256": {
            "standings": _sha256(configured_path(config, "standings")),
            "playoffs": _sha256(configured_path(config, "playoffs")),
            "team_mapping": _sha256(configured_path(config, "team_mapping")),
        },
        ...
        "methodology": {
            ...
            "team_names": "Historical franchise names are normalized using Data Collection's published franchise_lineage.csv before report generation.",
    ...
    markdown_lines = [
        ...
        "",
        f"- Run ID: `{reports_dir.name}`",
        f"- Generated locally: `{run_time.isoformat()}`",
        f"- Generated UTC: `{run_time.astimezone(timezone.utc).isoformat()}`",
        "",
        ...
        "- Team names are normalized with Data Collection's published `franchise_lineage.csv` before reports are created.",
...
def generate_reports(config: dict | None = None, run_name: str | None = None) -> dict[str, pd.DataFrame]:
    """Generate one immutable timestamped report run and update the latest pointer."""
    ...
    reports_dir, run_time = create_run_directory(reports_root, run_name=run_name)
    ...
    write_ai_readable_outputs(reports_dir, report_specs, reports, thresholds, config, run_time)
    write_latest_pointer(reports_root, reports_dir, datetime.now().astimezone())
...
def main() -> None:
    ...
    generate_reports(config, run_name=args.run_name)
