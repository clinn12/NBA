"""Pull and publish historical NBA playoff results.
...
Purpose
-------
Retrieve Basketball Reference playoff standings, preserve raw HTML by season,
validate champion flags, and update the shared historical publication.

Usage
-----
Run ``python scripts/pull_historical_playoffs.py`` from any directory. The
default config publishes to ``data/published/historical/nba_playoffs.csv``.

This script retrieves Basketball Reference playoff standings and updates
the governed shared publication. It replaces manual notebook execution with a repeatable
...
def fetch_playoff_html(year: int, timeout: int = 30, raw_path: Path | None = None) -> BeautifulSoup:
    """Load cached raw HTML or retrieve and preserve one playoff page."""
...
def update_playoffs(config: dict | None = None, *, write_manifest: bool = True) -> pd.DataFrame:
    ...
    raw_root = configured_path(config, "raw_root") / "playoffs"
    ...
    for year in range(start_year, last_year + 1):
        ...
        raw_path = raw_root / f"NBA_{year}_playoffs.html" if reuse_raw else None
        soup = fetch_playoff_html(year, timeout=timeout, raw_path=raw_path)
