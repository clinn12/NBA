...
ACTIVE_PYTHON_ROOTS = ("data_collection", "scripts", "tests")
ACTIVE_NOTEBOOKS: tuple[Path, ...] = (
    Path("notebooks/Pull_NBA_Standings.ipynb"),
    Path("notebooks/Pull_NBA_Playoffs.ipynb"),
)
REQUIRED_MODULE_SECTIONS = ("purpose", "usage")
REQUIRED_NOTEBOOK_SECTIONS = ("purpose", "how to use")
REQUIRED_PYTHON_FRONT_MATTER = (
    "front matter", "project: nba data collection", "file type:",
...
def audit_python_file(path: Path) -> List[str]:
    ...
    # Legacy notebooks may retain a UTF-8 byte-order mark after migration from
    # PowerShell/Jupyter; utf-8-sig accepts it without weakening JSON validation.
...
def audit_markdown(path: Path) -> List[str]:
    ...
    if fields.get("project") != "NBA Data Collection":
        violations.append(f"{relative}: project must be NBA Data Collection")
...
def audit_notebook(path: Path) -> List[str]:
    ...
    relative = path.relative_to(PROJECT_ROOT).as_posix()
    # Migrated Jupyter files can legitimately retain a UTF-8 byte-order mark.
    source_text = path.read_text(encoding="utf-8-sig")
    ...
    if visible_fields.get("project") != "NBA Data Collection" or metadata.get("project") != "NBA Data Collection":
        violations.append(f"{relative}: visible and notebook metadata project must be NBA Data Collection")
    if any(reference in source_text.lower() for reference in FORBIDDEN_PROJECT_REFERENCES):
        violations.append(f"{relative}: contains an obsolete former-project reference")
    return violations
